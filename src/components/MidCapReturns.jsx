...
const MidCapReturns = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:5000/get_mid_cap_mutual_funds_${type}`);
        let responseData;
  ...
  return (
    <div>
      <h2>Mid Cap Equity Funds based on   {type === '1Y' ? '1 Year' : type === '3Y' ? '3 Year' : type === '5Y' ? '5 Year' : type==='10Y' ? '10 Year' :'Unknown'} Returns </h2>
